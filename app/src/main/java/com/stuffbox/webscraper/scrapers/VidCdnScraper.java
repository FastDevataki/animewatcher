...
public class VidCdnScraper {
    ...
    public VidCdnScraper(Document gogoAnimePageDocument) {
    ...
    public ArrayList<Quality> getQualityUrls()
    {
        ...
        try {
            ...
            if(!m3u8Link.equals(""))
            {
               ...
               // Log.i("m3u8html",m3u8PageDocument.outerHtml());
                ...
                while (qualityMatcher.find() && m3u8Matcher.find())
                {
                   String qualityUrl = base+"/" + htmlToParse.substring(m3u8Matcher.start(),m3u8Matcher.end());
                    ...
                    Log.i("m3u8qualityurl",qualityUrl);
                    Log.i("m3u8quality",quality);
                    qualities.add(new Quality(quality,qualityUrl));
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
            Log.i("m3u8html",e.getMessage());
        }
    return qualities;
    }
